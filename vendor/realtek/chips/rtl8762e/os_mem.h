...
void *__rtl_os_mem_alloc_intern(RAM_TYPE ram_type, size_t size,
                          const char *p_func, uint32_t file_line);

void *__rtl_os_mem_zalloc_intern(RAM_TYPE ram_type, size_t size,
                           const char *p_func, uint32_t file_line);

void *__rtl_os_mem_aligned_alloc_intern(RAM_TYPE ram_type, size_t size, uint8_t alignment,
                                  const char *p_func, uint32_t file_line);
...
#define os_mem_alloc(ram_type, size)    \
    __rtl_os_mem_alloc_intern(ram_type, size, __func__, __LINE__)
...
#define os_mem_zalloc(ram_type, size)   \
    __rtl_os_mem_zalloc_intern(ram_type, size, __func__, __LINE__)
 ...
 *                          must use __rtl_os_mem_aligned_free() API function to free.
 *
 * \return     The address of the allocated memory block. If the address is NULL, the
 *             memory allocation failed.
 *
 ...
 *         __rtl_os_mem_aligned_free(p_mem);
 *     }
 *     else
 *     {
...
#define os_mem_aligned_alloc(ram_type, size, alignment) \
    __rtl_os_mem_aligned_alloc_intern(ram_type, size, alignment, __func__, __LINE__)
 ...
 * \ingroup  Memory
 */
void __rtl_os_mem_free(void *p_block);
 ...
 * \ingroup  Memory
 */
void __rtl_os_mem_aligned_free(void *p_block);
 ...
 *     unused_size = __rtl_os_mem_peek(RAM_TYPE_DATA_ON);
 *
 *     // Peek unused buffer on memory size.
 *     unused_size = __rtl_os_mem_peek(RAM_TYPE_BUFFER_ON);
 *
 *     return 0;
 * }
 * \endcode
 ...
 * \ingroup  Memory
 */
size_t __rtl_os_mem_peek(RAM_TYPE ram_type);

/* internal functions */
void __rtl_os_mem_check_heap_usage(void);

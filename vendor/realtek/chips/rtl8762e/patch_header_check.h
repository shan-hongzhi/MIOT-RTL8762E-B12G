...
typedef struct
{
    ...
    uint16_t crc16; //!<when enable image integrity check in boot flow, crc16 check image integrity if crc16 is not zero, otherwise __RTL_SHA256 check.
...
*/
extern uint32_t __rtl_get_active_ota_bank_addr(void);
...
*/
extern bool __rtl_is_ota_support_bank_switch(void);

/**
 * @brief  calculated checksum(CRC16 or __RTL_SHA256 determined by image) over the image, and compared
 ...
 * @retval false otherwise
*/
extern bool __rtl_check_image_chksum(T_IMG_CTRL_HEADER_FORMAT *p_header);
/**
 ...
 * @retval false otherwise
*/
extern bool __rtl_check_header_valid(uint32_t header_addr, T_IMG_ID image_id);
/**
 * @brief  get specified image header address in active bank
 * @param  image_id specify the image
 * @return specified image header address
*/
extern uint32_t __rtl_get_header_addr_by_img_id(T_IMG_ID image_id);
...
*/
extern uint32_t __rtl_get_active_bank_image_size_by_img_id(T_IMG_ID image_id);
/**
...
*/
extern uint32_t __rtl_get_temp_ota_bank_addr_by_img_id(T_IMG_ID image_id);
...
*/
extern uint32_t __rtl_get_temp_ota_bank_size_by_img_id(T_IMG_ID image_id);
...
*/
extern bool __rtl_get_active_bank_image_version(T_IMG_ID image_id, T_IMAGE_VERSION *p_image_version);

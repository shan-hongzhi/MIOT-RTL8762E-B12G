...
void ADC_Init(ADC_TypeDef *ADCx, ADC_InitTypeDef *ADC_InitStruct)
{
    ...
    reg_value = __rtl_btaon_fast_read_safe(0x116);
    __rtl_btaon_fast_write_safe(0x116, reg_value | BIT(10));

    /* vref_sel = 0.85V */
    reg_value = __rtl_btaon_fast_read_safe(0x116);
    __rtl_btaon_fast_write_safe(0x116, reg_value | BIT(5) | BIT(4));
...
void ADC_Cmd(ADC_TypeDef *ADCx, uint8_t adcMode, FunctionalState NewState)
{
    ...
    if (NewState == ENABLE)
    {
        reg_value = __rtl_btaon_fast_read_safe(0x113);
        __rtl_btaon_fast_write_safe(0x113, reg_value & ~BIT2);
        /* In case manual mode */
        if (ADCx->PWRDLY  & ADC_POWER_ON_MANUAL)
        {
            ADCx->PWRDLY  |= 0x3C00;
//            ADC_DelayUs(80);
            __rtl_platform_delay_us(80);
            ADCx->PWRDLY  |= (BIT14 | BIT15);
//            ADC_DelayUs(320);
            __rtl_platform_delay_us(320);
            ADCx->PWRDLY  |= (BIT16 | BIT17);
//            ADC_DelayUs(240);
            __rtl_platform_delay_us(240);
            ADCx->PWRDLY  |= BIT18;
        }
...
void ADC_BypassCmd(uint8_t ChannelNum, FunctionalState NewState)
{
    ...
    if (NewState != DISABLE)
    {
        reg_value = __rtl_btaon_fast_read_safe(0x118);
        reg_value |= BIT(ChannelNum);
        __rtl_btaon_fast_write_safe(0x118, reg_value);
    }
    else
    {
        reg_value = __rtl_btaon_fast_read_safe(0x118);

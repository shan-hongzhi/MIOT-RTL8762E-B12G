 ...
 * \ingroup  Synchronization
 */
void __rtl_os_unlock(uint32_t s);
 ...
 *          This number of available resources is specified as parameter of the __rtl_os_sem_create()
 *          function. Each time a semaphore token is obtained with __rtl_os_sem_take(), the semaphore
 ...
 *          is free. Semaphores are released with __rtl_os_sem_give() incrementing the semaphore count.
 *
 ...
 *     if (__rtl_os_sem_create(&p_handle, 0, 10) == true)
 *     {
 ...
 * \ingroup  Synchronization
 */
bool __rtl_os_sem_create(void **pp_handle, uint32_t init_count, uint32_t max_count);
 ...
 * \ingroup  Synchronization
 */
bool __rtl_os_sem_delete(void *p_handle);
 ...
 *     __rtl_os_sem_create(&p_handle, 1, 1);
 * }
 *
 ...
 * \ingroup  Synchronization
 */
bool __rtl_os_sem_take(void *p_handle, uint32_t wait_ms);
 ...
 *     __rtl_os_sem_create(&p_handle, 0, 1);
 *
 *     // Obtaining the empty semaphore immediately will be failed.
 *     if (__rtl_os_sem_take(p_handle, 0) == false)
 *     {
 *         // Failed.
 *     }
 *
 *     // Give the sempahore
 *     if (__rtl_os_sem_give(p_hanel) == true)
 *     {
 *         // Now we can take the semaphore.
 *         __rtl_os_sem_take(p_handle, 0);
 *
 ...
 * \ingroup  Synchronization
 */
bool __rtl_os_sem_give(void *p_handle);
 ...
 *
 *     // Create a mutex.
 *     if (__rtl_os_mutex_create(&p_handle) == true)
 *     {
 ...
 * \ingroup  Synchronization
 */
bool __rtl_os_mutex_create(void **pp_handle);
 ...
 *     __rtl_os_mutex_delete(p_handle);
 *
 *     return 0;
 * }
 * \endcode
 ...
 * \ingroup  Synchronization
 */
bool __rtl_os_mutex_delete(void *p_handle);
 ...
 * {
 *     // Create a mutex.
 *     __rtl_os_mutex_create(&p_handle);
 * }
 *
 ...
 *     // not available, wait for 100ms.
 *     if (__rtl_os_mutex_take(p_handle, 100) == true)
 *     {
 *         // Access the share resource.
 *
 *         // In real code, recursive calls of mutex may occur.
 *         __rtl_os_mutex_take(p_handle, 100);
 *         __rtl_os_mutex_take(p_handle, 200);
 *
 *         // The mutex has now been 'taken' three times, so will not be
 *         // available to another task until it has also been given back
 *         // three times.
 *         __rtl_os_mutex_give(p_handle);
 *         __rtl_os_mutex_give(p_handle);
 *         __rtl_os_mutex_give(p_handle);
 *
 *         // Finish accessing the share resource, then release the semaphore.
 ...
 * \ingroup  Synchronization
 */
bool __rtl_os_mutex_take(void *p_handle, uint32_t wait_ms);
 ...
 * \ingroup  Synchronization
 */
bool __rtl_os_mutex_give(void *p_handle);

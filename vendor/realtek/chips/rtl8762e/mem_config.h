...
example:
   ...
   2) Patch:                           15K (fixed)
   3) app global + ram code:           24K (adjustable, config APP_GLOBAL_SIZE)
   4) Heap ON:                         33K (adjustable, config HEAP_DATA_ON_SIZE)
*/
...
#define HEAP_DATA_ON_SIZE              (59 * 1024 - APP_GLOBAL_SIZE)
...
#define APP_GLOBAL_ADDR                (DATA_RAM_START_ADDR + DATA_RAM_RESERVED_SIZE)
...
#define DATA_ON_END_ADDR               (DATA_RAM_START_ADDR + DATA_RAM_TOTAL_SIZE)

...
extern void __rtl_log_direct(uint32_t info, const char *fmt, ...);

#define log_direct_retarget     __rtl_log_direct

/* Internal function that is used by internal macro DBG_LOWERSTACK. */
void __RTL_LogBufferLowerStack(uint32_t control, uint16_t log_str_index, uint8_t param_num, ...);

/* Internal function that is used by internal macro DBG_LOWERSTACKDATA. */
void __RTL_LogBufferLowerStackData(uint32_t control, uint16_t log_str_index, uint16_t length,
                             uint8_t *p_str);

/* Internal function that is used by internal macro DBG_BUFFER. */
void __rtl_log_buffer(uint32_t info, uint32_t log_str_index, uint8_t param_num, ...);

/* Internal function that is used by internal macro DBG_INDEX. */
void __rtl_log_index(uint32_t info, uint32_t log_str_index, uint8_t param_num, ...);

/* Internal function that is used by internal macro DBG_SNOOP. */
void __rtl_log_snoop(uint32_t info, uint16_t length, uint8_t *p_snoop);

/* Internal function that is used by public macro TRACE_BDADDR. */
const char *__rtl_trace_bdaddr(uint32_t info, char *bd_addr);

/* Internal function that is used by public macro TRACE_STRING. */
const char *__rtl_trace_string(uint32_t info, char *p_data);

/* Internal function that is used by public macro TRACE_BINARY. */
const char *__rtl_trace_binary(uint32_t info, uint16_t length, uint8_t *p_data);
...
#define DBG_BUFFER_INTERNAL(type, sub_type, module, level, fmt, param_num, ...) do {\
        ...
        __rtl_log_buffer(COMBINE_TRACE_INFO(type, sub_type, module, level), (uint32_t)format, param_num, ##__VA_ARGS__);\
    } while (0)
...
#define DBG_INDEX_LEVEL_ERROR(type, sub_type, module, fmt, param_num, ...) do {\
        __rtl_log_index(COMBINE_TRACE_INFO(type, sub_type, module, LEVEL_ERROR), fmt, param_num, ##__VA_ARGS__);\
    } while (0)
#else
...
#define DBG_INDEX_LEVEL_WARN(type, sub_type, module, fmt, param_num, ...) do {\
        __rtl_log_index(COMBINE_TRACE_INFO(type, sub_type, module, LEVEL_WARN), fmt, param_num, ##__VA_ARGS__);\
    } while (0)
#else
...
#define DBG_INDEX_LEVEL_INFO(type, sub_type, module, fmt, param_num, ...) do {\
        __rtl_log_index(COMBINE_TRACE_INFO(type, sub_type, module, LEVEL_INFO), fmt, param_num, ##__VA_ARGS__);\
    } while (0)
#else
...
#define DBG_INDEX_LEVEL_TRACE(type, sub_type, module, fmt, param_num, ...) do {\
        __rtl_log_index(COMBINE_TRACE_INFO(type, sub_type, module, LEVEL_TRACE), fmt, param_num, ##__VA_ARGS__);\
    } while (0)
#else
...
#define DBG_DIRECT(...)     do {\
        __rtl_log_direct(COMBINE_TRACE_INFO(TYPE_BEE3, SUBTYPE_DIRECT, 0, 0), __VA_ARGS__);\
    } while (0)

#define DBG_LOWERSTACK(color, file_num, line_num, log_str_index, param_num, ...)     do {\
        __RTL_LogBufferLowerStack(((file_num)<<16)|(line_num), (uint16_t)(log_str_index), param_num, ##__VA_ARGS__);\
    } while (0)

#define DBG_LOWERSTACKDATA(color, file_num, line_num, log_str_index, length, str)     do {\
        __RTL_LogBufferLowerStackData(((color)<<24)|((file_num)<<16)|(line_num), (uint16_t)(log_str_index), length, str);\
    } while (0)

#define DBG_SNOOP(type, sub_type, module, level, length, snoop)    do {\
        __rtl_log_snoop(COMBINE_TRACE_INFO(type, sub_type, module, level), length, snoop);\
    } while (0)


extern bool __rtl_is_log_init;
 ...
 * \ingroup  TRACE
 */
void __rtl_log_module_trace_init(uint64_t mask[LEVEL_NUM]);
 ...
 * \ingroup  TRACE
 */
bool __rtl_log_module_trace_set(T_MODULE_ID module_id, uint8_t trace_level, bool set);
 ...
 * \ingroup  TRACE
 */
bool __rtl_log_module_bitmap_trace_set(uint64_t module_bitmap, uint8_t trace_level, bool set);
...
#define TRACE_BDADDR(bd_addr)   \
    __rtl_trace_bdaddr(COMBINE_TRACE_INFO(TYPE_BEE3, SUBTYPE_BDADDR, 0, 0), (char *)(bd_addr))
...
#define TRACE_STRING(data)    \
    __rtl_trace_string(COMBINE_TRACE_INFO(TYPE_BEE3, SUBTYPE_STRING, 0, 0), (char *)(data))
...
#define TRACE_BINARY(length, data)  \
    __rtl_trace_binary(COMBINE_TRACE_INFO(TYPE_BEE3, SUBTYPE_BINARY, 0, 0), length, (uint8_t *)(data))
/**@}*/

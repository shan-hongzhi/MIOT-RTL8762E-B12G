                ...
                PAD_OUTPUT_VAL AON_PAD_O)
{
    ...
    tmpVal = __rtl_btaon_fast_read_safe(addr);
    ...
    __rtl_btaon_fast_write_safe(addr, tmpVal | SHDN);

    /* Pad control mode */
    if (AON_PAD_PwrOn == PAD_NOT_PWRON)
    {
        tmpVal &= ~SHDN;
        __rtl_btaon_fast_write_safe(addr, tmpVal);
    }
...
{
    uint16_t  tmpVal;
    tmpVal = __rtl_btaon_fast_read_safe(0x2a);
    /* clear reg value first */
    tmpVal &= ~(0x3f00);
    /* set value */
    tmpVal |= (time & 0x3f) << 8;
    ...
    tmpVal |= (BIT15 | BIT14);
    __rtl_btaon_fast_write_safe(0x2a, tmpVal);
}
/**
...
uint8_t Pad_WakeupInterruptValue(uint8_t Pin_Num)
{
    ...
    temp_value = __rtl_btaon_fast_read_safe(reg_temp);
...
uint8_t Pad_DebounceWakeupStatus(void)
{
    ...
    value16 = __rtl_btaon_fast_read_safe(0x2a);
    if (value16 & BIT15)
    {
        status_value = SET;
    }
    //Write 1 to clear debounceWakeupStatus
    __rtl_btaon_fast_write_safe(0x2a, (value16 | BIT15));
    return status_value;
}
...
void Pad_ClearWakeupINTPendingBit(uint8_t Pin_Num)
{
    ...
    reg_temp = (WKSTATUS_TABLE[Pin_Num] & ~(0xf000));
    bit_temp = BIT((WKSTATUS_TABLE[Pin_Num]  & (0xf000)) >> 12);
    reg_value = __rtl_btaon_fast_read_safe(reg_temp) | bit_temp;
    __rtl_btaon_fast_write_safe(reg_temp, reg_value);
}
...
void Pad_ClearAllWakeupINT(void)
{
    ...
    tmpVal = __rtl_btaon_fast_read_safe(0x9a);
    tmpVal |= 0x1BFF;
    __rtl_btaon_fast_write_safe(0x9a, tmpVal);

    tmpVal = __rtl_btaon_fast_read_safe(0xae);
    tmpVal |= 0x7F;
    __rtl_btaon_fast_write_safe(0xae, tmpVal);

    tmpVal = __rtl_btaon_fast_read_safe(0xd2);
    tmpVal |= 0x1FF;
    __rtl_btaon_fast_write_safe(0xd2, tmpVal);

    tmpVal = __rtl_btaon_fast_read_safe(0xf4);
    tmpVal |= 0x3FF;
    __rtl_btaon_fast_write_safe(0xf4, tmpVal);
}

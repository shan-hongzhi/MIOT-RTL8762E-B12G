...
//KEY_MASTER
/**
 * @brief    Gpio5IntrHandler for key interrupt
...
void Gpio4IntrHandler(void)
{
    ...
    if (keyMstatus == 0)
    {
        ...
        task_send_msg_to_app(&bee_io_msg);
    }
    ...
    if (keytype == MSG_KEYS_SHORT_PRESS) //salve short press
    {
        ...
        case PxpStateAdv:
            if (gSIoState == IoStateLlsAlert) // close beep
            {
                ...
                StartPxpSIO(ALERT_LOW_PERIOD * 2, ALERT_HIGH_PERIOD, LED_BLINK,
                            0xffffffff);
            }
            break;
...
void vTimerPxpSIOCallback(TimerHandle_t pxTimer)
{
    ...
    else
    {
        //set new period for display mode
        gSBeepflg = false;
        if (gPxpState == PxpStateAdv || gPxpState == PxpStateAdvScan || gPxpState == PxpStateLinkMADV)
        {
            gSIoState = IoStateAdvScanBlink;
            StartPxpSIO(ALERT_LOW_PERIOD * 2, ALERT_HIGH_PERIOD, LED_BLINK,

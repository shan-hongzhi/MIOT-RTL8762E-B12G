...
void print_flash_layout(void)
{
    ...
    APP_PRINT_INFO2("OTA Bank0: Addr=0x%x, size=0x%x", OTP->ota_bank0_addr, OTP->ota_bank0_size);
    ...
    APP_PRINT_INFO2("OTA Bank1: Addr=0x%x, size=0x%x", OTP->ota_bank1_addr, OTP->ota_bank1_size);
    ...
    APP_PRINT_INFO2("FTL: Addr=0x%x, size=0x%x", OTP->ftl_addr, OTP->ftl_size);
    APP_PRINT_INFO2("OTA TEMP: Addr=0x%x, size=0x%x", OTP->ota_tmp_addr, OTP->ota_tmp_size);
}

void show_otp_info(void)
{
    DBG_DIRECT("OTP: ftl_addr=0x%x, item addr=0x%x", OTP->ftl_addr, &OTP->ftl_addr);
    DBG_DIRECT("OTP: flash_wait_max=%d, item addr=0x%x", OTP->flash_wait_max, &OTP->flash_wait_max);
    DBG_DIRECT("OTP: ota_timeout_total=%d, item addr=0x%x", OTP->ota_timeout_total,
               &OTP->ota_timeout_total);
}
...
int main(void)
{
    ...
    uint8_t flash_bp_lv;
    flash_get_block_protect_locked(&flash_bp_lv);
    if (flash_bp_lv == 0)
    {
        DBG_DIRECT("[FLASH_BP] flash unlock");
    }
    else
    {
        DBG_DIRECT("[FLASH_BP] flash lock level is %d", flash_bp_lv);
    }

    DBG_DIRECT("Heap max block: data 0x%x, buffer 0x%x", os_mem_max_free_block_peek(RAM_TYPE_DATA_ON),
               os_mem_max_free_block_peek(RAM_TYPE_BUFFER_ON));

    DBG_DIRECT("CPU clk=%dM, ic_type=0x%02x", get_cpu_clock() / 1000000, get_ic_type());

    APP_PRINT_INFO3("FEATURE_RAM_CODE = %d, SHARE_CACHE_RAM_SIZE = 0x%x, OTP->ota_with_encryption_data = %d",
                    FEATURE_RAM_CODE, SHARE_CACHE_RAM_SIZE, OTP->ota_with_encryption_data);

    uint8_t bt_bd_addr[6];
    gap_get_param(GAP_PARAM_BD_ADDR, bt_bd_addr); //get OTP->bt_bd_addr
    APP_PRINT_INFO1("BDADDR:%b", TRACE_BINARY(6, bt_bd_addr));

#if (FLASH_256M_TEST == 1)
    if (flash_ioctl(0x0008, 0, 0) == 0)
    {
        //from 3 byte addr mode enter 4 byte addr mode
        flash_ioctl(0x100B, ENABLE, 0);
    }
#endif

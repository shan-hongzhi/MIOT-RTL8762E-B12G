    ...
    case IO_MSG_TYPE_KEYSCAN:
        {
            ...
            {
                T_KEYSCAN_FIFO_DATA *p_key_data = io_msg.u.buf;
                APP_PRINT_INFO0("KeyScan MSG, Sub type Scan end.");
                APP_PRINT_INFO1("[IO_MSG_TYPE_KEYSCAN] keyscan FIFO length is %d", p_key_data->len);
                for (uint8_t index = 0; index < (p_key_data->len); index++)
                {
                    APP_PRINT_INFO3("[IO_MSG_TYPE_KEYSCAN] keyscan data[%d]: row - %d, column - %d", \
                                    index, p_key_data->key[index].row, p_key_data->key[index].column);
                }
...
void app_handle_conn_state_evt(uint8_t conn_id, T_GAP_CONN_STATE new_state, uint16_t disc_cause)
{
    ...
    case GAP_CONN_STATE_DISCONNECTED:
        {
            APP_PRINT_INFO0("[app_handle_conn_state_evt] DISCONNECT");

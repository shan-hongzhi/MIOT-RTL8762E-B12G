...
void set_passkey(uint8_t key_num, PASSKEY_DEF *p_passkey)
{
    ...
    APP_PRINT_INFO2("passkey_value = %d,passkey_count = %d", p_passkey->value, p_passkey->count);
}

/******************************************************************
 * @brief    handle pair event
 */
void keyboard_handle_pair_event(T_KEYSCAN_FIFO_DATA *p_key_data)
{
    uint8_t usage_id;
 ...
 */
void keyboard_handle_press_event(T_KEYSCAN_FIFO_DATA *p_key_data)
{
    uint8_t keycount = 0;
    uint16_t usage_id;

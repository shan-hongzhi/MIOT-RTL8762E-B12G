...
#if defined(CONFIG_MIBLE_HAVE_MSC)
#include "mjac.h"
#endif
...
#define PRODUCT_ID                      5988//8059
#define MODEL_NAME                      "miot.light.m06071"//"miot.light.tmp5"
...
#if defined(CONFIG_MIBLE_USE_MCU_OTA)
static uint8_t mcu_version_buffer[2];
static void *mcu_ver_report_timer = NULL;
#endif
/* Config WatchDog*/
static int fd_wd;
...
#if defined(CONFIG_MIBLE_HAVE_MSC)
    { &mjax_msc_fd,    "/dev/i2c0", O_RDONLY, NULL, 0 },
#endif
};
...
#define CT2WARM_BRIGHTNESS(ct, ln)      ((ln) ? ((((ct) - 2700) * ln) / (6500 - 2700)) : 0)

light_demo_t xiaomi_light = {
    .light_on = true,
    .lightness = 30000,
    .lighttemp = 4000,
    .lightness_last = 30000,
    .lighttemp_last = 4000,
    .gmt_offset = 0,
    .weather = 0,
    .utc_time = 0,
...
{
    /** cold */
    {P2_4, TIM_PWM4, TIM4, TRUE, DEFAULT_LIGHTNESS, DEFAULT_LIGHTNESS},
    /** warm: set to actual pin*/
    {P2_5, TIM_PWM5, TIM5, TRUE, DEFAULT_LIGHTNESS, DEFAULT_LIGHTNESS},
};
...
{
    /* CONFIG GPIOS */
    for (uint8_t i = 0; i < sizeof(light_cw) / sizeof(light_t); ++i)
    {
        light_pin_config(&light_cw[i]);
    }
...
static void light_reset_change_done(light_t *light)
{
    miio_system_reboot();
}
...
{
    if(xiaomi_light.lightness > 0) {
        xiaomi_light.lightness_last = xiaomi_light.lightness;
    }
    xiaomi_light.lighttemp_last = xiaomi_light.lighttemp;

    printk("light_lightness_change_done lightness %d, temp %d\n",
        xiaomi_light.lightness_last, xiaomi_light.lighttemp_last);
}

int light_onoff_set(bool on)
{
    uint16_t bright_warm = CT2WARM_BRIGHTNESS(xiaomi_light.lighttemp_last,
                          xiaomi_light.lightness_last);
    uint16_t bright_cold = xiaomi_light.lightness_last - bright_warm;

    printk("light_onoff_set %d\n", on);
    // ioctl(gpio_fd, GPIOC_WRITE, on);

    xiaomi_light.light_on = on;

    if (on) {
        light_set_lightness_linear(&light_cw[0], bright_cold, 500, NULL);
        light_set_lightness_linear(&light_cw[1], bright_warm, 500, light_lightness_change_done);
        xiaomi_light.lightness = xiaomi_light.lightness_last;
    } else {
        light_set_lightness_linear(&light_cw[0], 0, 500, NULL);
        light_set_lightness_linear(&light_cw[1], 0, 500, light_lightness_change_done);
        xiaomi_light.lightness = 0;
    }

    return 0;
}

int light_lightness_set(uint16_t lightness)
{
    uint16_t bright_warm = CT2WARM_BRIGHTNESS(xiaomi_light.lighttemp,
                          lightness);
    uint16_t bright_cold = lightness - bright_warm;

    xiaomi_light.lightness = lightness;
...
int light_temperature_set(uint16_t lighttemp)
{
    uint16_t bright_warm = CT2WARM_BRIGHTNESS(lighttemp,
                          xiaomi_light.lightness);
    uint16_t bright_cold = xiaomi_light.lightness - bright_warm;

    xiaomi_light.lighttemp = lighttemp;
...
static void find_user_update_param_update(struct bt_conn *conn, void *data)
{
    if (conn->state == BT_CONN_CONNECTED)
    {
        struct bt_le_conn_param param;
        param.interval_min = 6;
        param.interval_max = 9;
        param.latency = 0;
        param.timeout = 400;
        int err;
...
static void user_update_param_update(void)
{
    bt_conn_foreach(BT_CONN_TYPE_ALL, find_user_update_param_update, NULL);
}
...
#if defined(CONFIG_MIBLE_USE_MCU_OTA)
void main_mcu_version_report_handler(void * arg)
{
    NET_BUF_SIMPLE_DEFINE(msg, 8);
    uint8_t mcu_ver[20];
    int status = 0;

    struct mible_mesh_msg_sent_ctx ctx_rsp = {
        .opcode = 0x00FF,
        .delay = 30,
        .dst = 0xFEFF,
        .mod_idx = 0,
    ...
    net_buf_simple_add_u8(&msg, 0x06);
    net_buf_simple_add_u8(&msg, MIBLE_LIB_MAJOR);
    net_buf_simple_add_u8(&msg, MIBLE_LIB_MINOR);
    net_buf_simple_add_u8(&msg, MIBLE_LIB_REVISION);
    uint16_t dev_version = mi_config.cfg.dev_version;
    net_buf_simple_add_u8(&msg, (uint8_t)(dev_version&0x00ff));
    net_buf_simple_add_u8(&msg, (uint8_t)((dev_version>>8)&0xff));
    net_buf_simple_add_u8(&msg, mi_config.cfg.mcu_version[0]);
    net_buf_simple_add_u8(&msg, mi_config.cfg.mcu_version[1]);

    status = mible_mesh_msg_send(&ctx_rsp, &msg);
    printk("report mcu version err %d,msg len %d, size %d\n", status, msg.len, msg.size);
}

uint32_t main_rand(uint32_t down, uint32_t up)
{
    uint32_t val, diff = up - down;
    int err;

    err = bt_rand(&val, sizeof(val));
    if (err) {
        return down;
    }
...
void main_mcu_version_report(void)
{
    uint32_t random = main_rand(20,30);
    printk("MCU version report after random:%ld(s)\n",random);
    uint32_t start_timeout = random * 1000;
    if(mcu_ver_report_timer !=NULL){
        miio_timer_start(mcu_ver_report_timer, start_timeout, NULL);
    }
}
#endif
...
static void user_event_cb(mible_user_event_type_t event, void *user_data)
{
    ...
    {
        is_provisioned = true;
        xiaomi_light.light_on = true;
        light_onoff_set(xiaomi_light.light_on);
        {
            int bright_warm = CT2WARM_BRIGHTNESS(xiaomi_light.lighttemp, xiaomi_light.lightness);
            int bright_cold = xiaomi_light.lightness_last - bright_warm;
            light_breath(&light_cw[0], 0, bright_cold, 500, 50, 3, true, NULL);
            light_breath(&light_cw[1], 0, bright_warm, 500, 50, 3, true, NULL);
        }
        //keep adv always on
        miio_system_set_adv_timeout(0xFFFFFFFF);
#if TEST_GATT_SPEC
        miio_mesh_properties_period_publish_init(3, 1, 0);
        miio_mesh_properties_period_publish_init(3, 2, 0);
#endif
#if defined(CONFIG_MIBLE_USE_MCU_OTA)
        main_mcu_version_report();
#endif
    }break;
    ...
    case MIBLE_USER_PROV_CB_TYPE_RESET:{
        bool *reboot = user_data;
        *reboot = false;
        light_breath(&light_cw[0], 0, 30000, 1000, 50, 3, false, NULL);
        light_breath(&light_cw[1], 0, 30000, 1000, 50, 3, false, light_reset_change_done);
    }break;
...
{
    mible_spec_property_value_new_boolean(&o->value, xiaomi_light.light_on);
}

static void P_2_2_Brightness_doGet(mible_spec_bearer_t bearer, mible_spec_property_operation_t *o)
{
    mible_spec_property_value_new_ushort(&o->value, xiaomi_light.lightness);
}

static void P_2_3_ColorTemperature_doGet(mible_spec_bearer_t bearer,
                     mible_spec_property_operation_t *o)
{
    mible_spec_property_value_new_ushort(&o->value, xiaomi_light.lighttemp);
}
...
static void P_2_1_On_doSet(mible_spec_bearer_t bearer, mible_spec_property_operation_t *o)
{
    ...
    light_onoff_set(o->value.data.boolean);

    o->code = MIBLE_SPEC_OPERATION_OK;
}

static void P_2_2_Brightness_doSet(mible_spec_bearer_t bearer, mible_spec_property_operation_t *o)
{
    if (o->value.format != MIBLE_SPEC_PROPERTY_FORMAT_USHORT && o->value.format != MIBLE_SPEC_PROPERTY_FORMAT_NUMBER)
    {
        o->code = MIBLE_SPEC_OPERATION_ERROR_VALUE;
        return;
    }

    light_lightness_set(o->value.data.ushortValue);

    o->code = MIBLE_SPEC_OPERATION_OK;
}

static void P_2_3_ColorTemperature_doSet(mible_spec_bearer_t bearer,
                     mible_spec_property_operation_t *o)
{
    if (o->value.format != MIBLE_SPEC_PROPERTY_FORMAT_ULONG && o->value.format != MIBLE_SPEC_PROPERTY_FORMAT_NUMBER)
    {
        o->code = MIBLE_SPEC_OPERATION_ERROR_VALUE;
        return;
    }

    light_temperature_set(o->value.data.ulongValue);
...
static void P_2_1_On_doAction(mible_spec_bearer_t bearer, mible_spec_action_operation_t *o)
{
    ...
    if(xiaomi_light.light_on){
        xiaomi_light.light_on = false;
    }else{
        xiaomi_light.light_on = true;
    }
    light_onoff_set(xiaomi_light.light_on);

    mible_spec_property_value_new_boolean(&value, xiaomi_light.light_on);
...
static void button_process_handler(void *args)
{
    ...
    xiaomi_light.light_on = xiaomi_light.light_on? false:true;
    light_onoff_set(xiaomi_light.light_on);

    if(is_provisioned){
        mible_spec_property_value_t value;
        mible_spec_property_value_new_boolean(&value, xiaomi_light.light_on);
        mible_mesh_send_property_changed(2, 1, &value);
...
MIBLE_SPEC_DEFINE(spec_handlers)
    ...
    MIBLE_SPEC_SIID_START(2)
        ...
        MIBLE_SPEC_HANDLER(2, P_2_2_Brightness_doGet, P_2_2_Brightness_doSet, NULL),
        MIBLE_SPEC_HANDLER(3, P_2_3_ColorTemperature_doGet, P_2_3_ColorTemperature_doSet, NULL),
    MIBLE_SPEC_SIID_END
...
const volatile struct mible_config APP_CFG_INFO mi_config = {
    ...
    .cfg =
    {
...
#if defined(CONFIG_MIBLE_USE_MCU_OTA)
        .mcu_version = mcu_version_buffer,
#endif
        .gatt_spec =
        {
        ...
        #endif
        #if defined(CONFIG_MIBLE_USE_MCU_OTA)
            .mcu_ota_init = mible_dfu_mcu_ota_enable,
        #endif
        },
    },
    .cb =
...
static bool light_flash_restore(void)
{
    uint32_t pwr_cnt = 0;
    int ret;
...
#if defined(CONFIG_MIBLE_USE_MCU_OTA)
    if(mi_config.cfg.init.mcu_ota_init){
        mi_config.cfg.init.mcu_ota_init();
    }
    err =  mible_mcu_init();
    printk("mcu ota init (err %d)\n", err);

    if(err == 0){
        uint32_t delay = 0;
        uint8_t buf_ver[4] = {0};
        /*recommend to use delayed work to read mcu version;
        *  just use sync mode to read for demo
        */
        mible_mcu_send_get_info(&delay);
        mible_mcu_read_get_info(buf_ver);

        //mcu version report timer
        err = miio_timer_create(&mcu_ver_report_timer,
                main_mcu_version_report_handler,
                MIBLE_TIMER_SINGLE_SHOT);
        if (err < 0) {
            printk("Unable to create mcu ver report timer\n");
        }
    }
#endif

...
void user_scan_check(void)
{
    if(is_provisioned){
        ...
        // if need scan when unprov, set scan normal mode(15%)
        //miio_mesh_set_scan_level(MESH_SCAN_NORMAL);
        miio_mesh_set_scan_level(MESH_SCAN_OFF);
    }
}
...
static void button_process_handler(void *args)
{
    ...
    //test dram pmp
    write_sram32(0x80000, 0x1122334455);
}
...
const struct mible_config mi_config = {
    .cfg =
    {
        ...
        #if 0//defined(CONFIG_BT_MESH_PSK)
...
static void app_periodic_handler(void * arg)
{
    ...
    // int err = ioctl(fd_wd, WDIOC_KEEPALIVE, 0);
    // if(err < 0){

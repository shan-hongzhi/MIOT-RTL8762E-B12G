...
#define PWM_FREQ            12000                   //!< 12KHz
#define PWM_PCLK_SPEED      (PWM_FREQ*1000)
#define PWM_FUNC(id)        (id + 1)                //gpio_func_e
#define PWM_FUNC_N(id)      (id + 7)
#define PWM_ENABLE(id)      ((id)? BIT(id): BIT(8)) //pwm_en_e

void light_pin_config(const light_t *light)
{
    if(light == NULL || light->pin_num == 0xFF)
        return;

    /* pad & pinmux */
    pwm_set_pin(light->pin_num, PWM_FUNC(light->pwm_id));

    /*<! pwm_clk_div = pclk_frequency /pwm_frequency -1;（pwm_clk_div：0~255） */
    pwm_set_clk((unsigned char)(sys_clk.pclk * 1000 * 1000 / PWM_PCLK_SPEED - 1));
    /*<! PWM ferq = PWM_PCLK_SPEED/tmax */
    /*<! PWM duty cycle = tcmp/tmax */
    pwm_set_tmax(light->pwm_id, 1000);
    if (0xffff == light->lightness){
        pwm_set_tcmp(light->pwm_id, 1000);
    }else{
        pwm_set_tcmp(light->pwm_id, light->lightness/66);
    }
    if(!light->pin_high_on){
        pwm_invert_en(light->pwm_id);
    }
    /* Enable PWM output */
    pwm_start(PWM_ENABLE(light->pwm_id));

    printk("light_pin_config pin %d, id %d, clk %d, tcmp %d, enable %ld", light->pin_num, light->pwm_id,
        (unsigned char)(sys_clk.pclk * 1000 * 1000 / PWM_PCLK_SPEED - 1), light->lightness/66,
        PWM_ENABLE(light->pwm_id));
}
...
void light_lighten(light_t *light, uint16_t lightness)
{
    ...
    if (0xffff == lightness){
        ...
        pwm_set_tcmp(light->pwm_id, lightness/66);
    }

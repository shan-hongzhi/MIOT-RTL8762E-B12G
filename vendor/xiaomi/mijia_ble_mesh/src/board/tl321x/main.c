...
#define PRODUCT_ID                      8059
#define MODEL_NAME                      "miot.light.tmp5"

#define DEVELOPER_VERSION               204
...
{
    /** cold */
    {GPIO_PC2, PWM0_ID, TRUE, DEFAULT_LIGHTNESS, DEFAULT_LIGHTNESS},
    /** warm: set to actual pin*/
    {GPIO_PC3, PWM1_ID, TRUE, DEFAULT_LIGHTNESS, DEFAULT_LIGHTNESS},
};
...
const uint8_t model[] = MODEL_NAME;
...
uint8_t reset_reason_tl321x(void)
{
     /* Log MCU startup mode for debugging and system monitoring */
    uint8_t mcu_status = pm_get_mcu_status();
    printk( "MCU startup mode:0x%02x \n", mcu_status);

    /* Check if reset was triggered by software reboot */
    if (mcu_status == MCU_STATUS_POWER_ON) {
        return 0; /* HW reset */
    } else if (mcu_status == MCU_HW_REBOOT_TIMER_WATCHDOG || mcu_status == MCU_HW_REBOOT_32K_WATCHDOG) {
        return 1; /* WDG timeout */
    } else if(MCU_SW_REBOOT_BACK & mcu_status) {
        /* Read and log the specific reason for the software reboot */
        uint8_t reboot_reason = analog_read_reg8(PM_ANA_REG_POWER_ON_CLR_BUF0) && 0xFE;
        /* Note: A reason code of 0 indicates a planned reboot */
        printk("Soft reboot reason:0x%02x\n", reboot_reason);
        return 2; /* SW reset */
    }
    return 0;
}
...
#if defined(CONFIG_BT_MESH_STATISTIC)
    mible_trace_update_reboot_reason(reset_reason_tl321x());
    mible_trace_init();
#endif

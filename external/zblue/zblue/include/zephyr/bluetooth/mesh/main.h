...
/** @brief Register mesh path api.
 *
 * This API allows shoule be called before enable mesh path feature.
 *
 * @param nid_check check message net id
 * @param not_relay whether message on path or not (no need to relay other path message)
 * @param get_xmit  message need more retransmit or not
 */
void register_mesh_path_api_into_stack(
	int      (*nid_check)(struct bt_mesh_msg_ctx *ctx),
	int      (*not_relay)(struct bt_mesh_msg_ctx *ctx),
	int       (*get_xmit)(struct bt_mesh_msg_ctx *ctx));

/** @brief Modify mesh scan window.
 *
 *  Modify mesh scan window for low power node.
 *
 *  @param ratio 0 (per adv scan) ~ 20 (low power scan) ~ 100 (full window scan)
 *
 *  @return Zero on success or (negative) error code otherwise.
 */
int bt_mesh_scan_duty_cycle(uint8_t ratio);

/** @brief Inject scan data into stack.
 *
 *  Modify mesh scan window for low power node.
 *
 *  @param addr Device public address
 *  @param rssi Packet rssi
 *  @param adv_type Advertising PDU types
 *  @param buf  Received packet raw data
 */
void bt_mesh_scan_data_handler(const bt_addr_le_t *addr, int8_t rssi,
